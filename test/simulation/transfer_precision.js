...
const trackerGrowth = new Stochasm({ min: -0.5, max: 2.5, seed: 'fragments.org' });

let tracker, rebaseAmt, inflation, preRebaseSupply, postRebaseSupply;
...
async function checkBalancesAfterOperation (users, op, chk) {
  ...
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      _bals.push(await tracker.balanceOf.call(users[u]));
    }
  }
  await op();
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      bals.push(await tracker.balanceOf.call(users[u]));
    }
  }
  chk(_bals, bals);
}
...
async function checkBalancesAfterTransfer (users, tAmt) {
  await checkBalancesAfterOperation(users, async function () {
    await tracker.transfer(users[1], tAmt, { from: users[0] });
...
async function exec () {
  ...
  let i = 0;
  do {
    await tracker.rebase(i + 1, rebaseAmt, {from: deployer});
    ...
    const tAmt = (await tracker.balanceOf.call(deployer));
